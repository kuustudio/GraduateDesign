...
from Demo2.src.get_each_info import *
"""
 :@detail 获取企业详细信息
"""
def get_info(url, count):
    if count > retries:
        print("重试超过%d次：建议停机检查：" % retries, url)
        return True
    ...
    getConnection(soup = soup, url = url)


    # 公司背景
    div = soup.find('div', id="_container_baseInfo")
    if div is None:
        print("此页查找不到公司内容 _container_baseInfo：", url, "注意这个必须要查到！！！！！！！！！！！")
        # print(html)
        refresh_proxy_ip(1)
        ...
        if compChdiv is None:
            print("此页查找不到公司全称呼  class_=header：", url, "注意这个必须要查到！！！！但先不刷新IP！")
            compCh= ""
        else:
        ...
        fddbrr ="None"
        if frdiv is None:
            print("此页查找不到公司法人 humancompany：", url, "先进行第二轮查找")
            try:
                fddbrr = div.find('tr').find('span').text
                print("找到了公司法人 ：", fddbrr)
            except:
                print('此页没有公司法人', url, "注意这个必须要查到,但先不刷新IP！")
        else:
            fddbrr = frdiv.find('a', class_="link-click").get_text()  # 法定代表人

        table = div.find('table', class_="table -striped-col -breakall")
        if table is None:
            print("此页查找不到公司内容 [table -striped-col -breakall]：", url, "转为注销公司查找模式！")
            # print(html)
            if (count + 1 > retries):
                ...
                if (tds_else is None):
                    ...
                    print("找到注销公司内容！")
                    get_info_to_sql(tds_else, compCh = compCh, fddbrr = fddbrr, url = url, mode = 2)
            else:
                refresh_proxy_ip(1)
                return get_info(url, count + 1)
        else:
