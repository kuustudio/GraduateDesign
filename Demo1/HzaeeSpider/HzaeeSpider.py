...
class HzaeeSpider():
    ...
    def startDownload(self, mode = 1):
        print('产权转让：披露， 国有')
        self.__getCQZR_formal(guoyou=True, mode = mode)
        print('产权转让：非披露， 国有')
        self.__getCQZR_informal(guoyou=True, mode = mode)
        for type in self.__estate:
            print('资产转让：' + type + '， 国有')
            self.__getZCZR(guoyou=True, Type=type, mode = mode)
        #self.__getQYZZ(guoyou=True)
        #self.__getZS(True)
        print('房屋租赁， 国有')
        self.__getFWZL(True, mode = mode)
        #self.__getELSE(True)

        print('产权转让：披露， 非国有')
        self.__getCQZR_formal(guoyou=False, mode = mode)
        print('产权转让：非披露， 非国有')
        self.__getCQZR_informal(guoyou=False, mode = mode)
        for type in self.__estate:
            print('资产转让：' + type + '， 非国有')
            self.__getZCZR(guoyou=False, Type=type, mode = mode)
        #self.__getQYZZ(guoyou=False)
        #self.__getZS(False)
        print('房屋租赁， 非国有')
        self.__getFWZL(False, mode = mode)
    ...
    """
        ...
        产权转让
            正式披露
    """
    def __getCQZR_formal(self, guoyou, pageNum = 1, mode = 1):
        self.__params = {
            'page': str(pageNum),
            'limit': '15',
            'order': 'desc',
        ...
        for item in currentPageInfo:
            ...
            self.__dataFrame = hzaeeItem.item2csv_sql(self.__dataFrame, mode=mode)
            self.__CQZRs_formal.append(hzaeeItem)

        if info['totalPage'] > 1:
            if pageNum == info['totalPage']:
                return
            self.__getCQZR_formal(guoyou, pageNum=pageNum + 1, mode=mode)
        else:
            return
    ...
    """
        ...
        产权转让
            预批露
    """
    def __getCQZR_informal(self, guoyou, pageNum = 1, mode = 1):
        self.__params = {
            'page': str(pageNum),
            'limit': '15',
            'order': 'desc',
        ...
        if info['totalPage'] > 1:
            if pageNum == info['totalPage']:
                return
            self.__getCQZR_informal(guoyou, pageNum=pageNum + 1, mode=mode)
        else:
            return
    ...
    """
    def __getZCZR(self, guoyou, pageNum = 1, Type = '房产', mode = 1):
        self.__params = {
            'page': str(pageNum),
            'limit': '15',
            'order': 'desc',
        ...
        if info['totalPage'] > 1:
            if pageNum == info['totalPage']:
                return
            self.__getZCZR(guoyou, pageNum=pageNum + 1, Type = Type, mode=mode)
        else:
            return
    ...
    """
        ...
        企业增资
    """
    def __getQYZZ(self, guoyou, pageNum = 1, mode = 1):
        self.__params = {
            'page': str(pageNum),
            'limit': '15',
            'order': 'desc',
        ...
        if info['totalPage'] > 1:
            if pageNum == info['totalPage']:
                return
            self.__getQYZZ(guoyou, pageNum=pageNum + 1, mode=mode)
        else:
            return
    ...
    """
        ...
        房屋租赁
    """
    def __getFWZL(self, guoyou, pageNum = 1, mode = 1):
        self.__params = {
            'page': str(pageNum),
            'limit': '15',
            'order': 'desc',
        ...
        if info['totalPage'] > 1:
            if pageNum == info['totalPage']:
                return
            self.__getFWZL(guoyou, pageNum=pageNum + 1, mode=mode)
        else:
            return
    ...
    """
        ...
        招商信息
    """
    def __getZS(self, guoyou, pageNum = 1, mode = 1):
        ...
        if info['total'] > 15:
            ...
            if pageNum == wholePageNum:
                self.__basicUrl = 'https://www.hzaee.com/api/api/subjectmaininfo/page'
                return
            self.__getZS(guoyou, pageNum=pageNum + 1, mode=mode)
        else:
            self.__basicUrl = 'https://www.hzaee.com/api/api/subjectmaininfo/page'
            return
    ...
    """
        ...
        其他信息
    """
    def __getELSE(self, guoyou, pageNum = 1, mode = 1):
        self.__basicUrl = 'https://www.hzaee.com/api/api/project/page'
        self.__params = {
            'page': str(pageNum),
            'limit': '15',
        ...
        if info['total'] > 15:
            ...
            if pageNum == wholePageNum:
                self.__basicUrl = 'https://www.hzaee.com/api/api/subjectmaininfo/page'
                return
            self.__getELSE(guoyou, pageNum=pageNum + 1, mode=mode)
        else:
            self.__basicUrl = 'https://www.hzaee.com/api/api/subjectmaininfo/page'
            return
